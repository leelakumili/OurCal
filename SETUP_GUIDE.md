...
you're running an APK with one bundled in — the `.apk` from
[Releases](../../releases) has one built in, and so does a self-built one
where `./packaging/build-android.sh` printed **"bundling the OAuth client"**
when it copied one in from the working tree. A fresh install of that APK can
add an account and sign in to Google with no computer at all:
